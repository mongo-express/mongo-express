...
const router = async function (config) {
  ...
  appRouter.use(process.env.NODE_ENV === 'test' ? csrf({ ignoreMethods: ['GET', 'HEAD', 'OPTIONS', 'POST', 'PUT', 'DELETE'] })
    : csrf({ cookie: true }));
  ...
  // Server-side enforcement of readOnly and noDelete configuration
