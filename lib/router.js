...
const router = function (config) {
  ...
  appRouter.param('document', function (req, res, next, id) {
    ...
    try {
      ...
      try {
        // Is it a GUID?
        binData3 = new mongodb.Binary(new Buffer(id, 'base64'), 3);
        binData4 = new mongodb.Binary(new Buffer(id, 'base64'), 4);
      } catch (err) {
    ...
    if (obj_id || binData3 || binData4) {
      // passed id has successfully been turned into a valid ObjectID
      req.collection.findOne({ "$or" : [{ _id: obj_id}, { _id: binData3}, { _id: binData4}] }, function (err, doc) {
        if (err) {
          req.session.error = 'Error: ' + err;
          return res.redirect(res.locals.baseHref + 'db/' + req.dbName + '/' + req.collectionName);
        }
