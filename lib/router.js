...
const router = async function (config) {
  ...
  appRouter.param('document', async function (req, res, next, id) {
    ...
    try {
      ...
      if (('subtype' in req.query) && ALLOWED_SUBTYPES.includes(Number.parseInt(req.query.subtype, 10))) {
        _id = new mongodb.Binary(new Buffer.from(id.split('-').join(''), 'hex'), req.query.subtype);
      } else {
    ...
    const findStraighId = async function (id) {
      // No document found with obj_id, try again with straight id
      await req.collection.findOne({ _id: id }).then((doc) => {
        if (doc === null) {
    ...
    await req.collection.findOne({ _id }).then(async (doc) => {
      if (doc === null) {
        // No document found with obj_id, try again with straight id
        await findStraighId(id);
      } else {
        // Document found - send it back
        req.document = doc;
        res.locals.document = doc;
