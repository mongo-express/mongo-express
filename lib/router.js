    ...
    if (('subtype' in query)) {
      ...
      if (ALLOWED_SUBTYPES.has(subtype)) {
        if (subtype === mongodb.Binary.SUBTYPE_UUID) {
          return new mongodb.Binary(Buffer.from(id.replaceAll('-', ''), 'hex'), query.subtype);
        }
        // mongodb.Binary.SUBTYPE_UUID_OLD
...
const router = async function (config) {
  ...
  // Session middleware - required for flash messages and stateful auth
  // Sessions are used even with Basic auth to support flash messages (error/success notifications)
  ...
  // Rate limiting for authentication endpoints (Security: Prevents brute-force attacks)
  ...
  const loginLimiter = rateLimit({
    ...
    skip: (req) => req.path === config.healthCheck.path || req.path.startsWith('/public/'),
  });

  // Initialize pluggable authentication system (AFTER session middleware)
  // Issue #1733 & #1766: Refactored to use Passport.js with multiple strategies
  const authHandler = createAuthHandler(config);

  try {
    // Initialize passport with the configured strategy
    const passportInit = await authHandler.initialize();
    appRouter.use(passportInit);

    // Only use passport session middleware if strategy requires it
    // (form and oidc need sessions for user persistence, basic does not)
    if (authHandler.requiresSession()) {
      const sessionMiddleware = authHandler.sessionMiddleware();
      if (sessionMiddleware) {
        appRouter.use(sessionMiddleware);
      }
    }
  } catch (error) {
    console.error(pico.red('Failed to initialize authentication:'));
  ...
  // Form-based login routes (only if using form/local strategy)
  // Issue #1733: Provides HTML form that password managers can recognize
  if (authHandler.strategy === 'form' || authHandler.strategy === 'local') {
    // Login page (GET) - Show the login form
    ...
    // Login form submission (POST) - Process credentials
    // Rate limiting applied to prevent brute-force attacks
    appRouter.post('/login', loginLimiter, authHandler.getFormLoginHandler());
  ...
  // OIDC routes (only if using OIDC strategy)
  if (authHandler.strategy === 'oidc') {
    // Login route
    appRouter.get('/login', authHandler.getOidcLogin());

    // Callback route
    appRouter.get('/callback', authHandler.getOidcCallback());
  ...
  // Apply authentication middleware to all routes except health check, public assets, and login
  // This protects all mongo-express functionality while allowing public resources
  appRouter.use((req, res, next) => {
    // Skip auth for login page and public assets (CSS, JS, images)
    if (req.path === '/login' || req.path.startsWith('/public/')) {
      return next();
    }
    // All other routes require authentication
    return authHandler.authenticate()(req, res, next);
  });
  ...
  appRouter.all('*', async function (req, res, next) {
    ...
    res.locals.enableLogout = config.useOidcAuth || config.authStrategy === 'oidc' || config.authStrategy === 'form' || config.authStrategy === 'local';
