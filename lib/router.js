...
const router = async function (config) {
  ...
  //
  // Redirect to the configured base URL rather than res.locals.baseHref: the latter is
  // derived from req.originalUrl, and CodeQL flags feeding request-controlled data into
  // res.redirect. config.site.baseUrl is server-side configuration and cannot be influenced
  // by the caller.
