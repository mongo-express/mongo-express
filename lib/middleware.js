...
const middleware = async function (config) {
  ...
  app.locals.assets = JSON.parse(fs.readFileSync(fileURLToPath(new URL('../build-assets.json', import.meta.url)), 'utf8'));
  ...
  // App configuration
  // Express 5 defaults to the 'simple' query parser, which does not understand nested
  // keys: `?sort[name]=1` would arrive as `{ 'sort[name]': '1' }` instead of
  // `{ sort: { name: '1' } }`, silently breaking sorting, filtering and projections.
  app.set('query parser', 'extended');
