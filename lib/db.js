...
const connect = async function (config) {
  ...
  connectionData.clients = await Promise.all(connections.map(async (connectionInfo, index) => {
    ...
    try {
      ...
      // connectionString may be undefined here; masking it directly used to throw a
      // TypeError and hide the real connection error.
      const masked = String(connectionString ?? '<none>').replace(/(mongo.*?:\/\/.*?:).*?@/, '$1****@');
      console.error(`Could not connect to database using connectionString: ${masked}"`);
      throw error;
    }
  }));
