...
/**
 * Hide the password in a connection string before logging it.
 *
 * Index-based rather than the previous `/(mongo.*?:\/\/.*?:).*?@/`: that pattern chained
 * lazy quantifiers over uncontrolled input and CodeQL flagged it as polynomial
 * backtracking. It also threw a TypeError when the connection string was undefined,
 * hiding the very error it was meant to report.
 *
 * @param {string} [connectionString]
 * @returns {string} the connection string with the password replaced by ****
 */
export const maskConnectionString = function (connectionString) {
  if (!connectionString) {
    return '<none>';
  }
  ...
  const passwordStart = connectionString.indexOf(':', schemeEnd + 3);

  if (passwordStart === -1 || passwordStart > at) {
    return connectionString;
  }

  return `${connectionString.slice(0, passwordStart + 1)}****${connectionString.slice(at)}`;
};
...
const connect = async function (config) {
  ...
  connectionData.updateCollections = async function (dbConnection) {
    ...
    const names = Array.from(collections, (collection) => collection.name);
  ...
  connectionData.clients = await Promise.all(connections.map(async (connectionInfo, index) => {
    ...
    try {
      ...
      console.error(`Could not connect to database using connectionString: ${maskConnectionString(connectionString)}"`);
      throw error;
    }
  }));
