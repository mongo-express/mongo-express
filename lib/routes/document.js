...
const routes = function (config) {
  ...
  exp.addDocument = function (req, res) {
    ...
    req.collection.insertOne(docBSON, function (err) {
      if (err) {
  ...
  exp.updateDocument = function (req, res) {
    ...
    req.collection.updateOne(req.document, { $set: docBSON }, function (err) {
      if (err) {
  ...
  exp.deleteDocument = function (req, res) {
    ...
    req.collection.deleteOne(req.document, function (err) {
      if (err) {
        req.session.error = 'Something went wrong! ' + err;
        console.error(err);
        return res.redirect('back');
