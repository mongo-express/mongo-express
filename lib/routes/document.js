...
var routes = function (config) {
  ...
  exp.updateDocument = function (req, res) {
    ...
    req.collection.update(req.document, docBSON, { safe: true }, function (err) {
      ...
      // Equivalent of "req.document._id | json | safe | url_encode }}" of HTML, with encoding twice 
      var idUriJson = encodeURIComponent(encodeURIComponent(JSON.stringify(req.document._id)));
      res.redirect(res.locals.baseHref + 'db/' + req.dbName + '/' + req.collectionName + '/' + idUriJson);
    });
  };
