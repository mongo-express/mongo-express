import { toBSON, toString } from '../bson.js';
import { stringDocIDs } from '../filters.js';
...
const routes = function (config) {
  ...
  exp.viewDocument = function (req, res) {
    const ctx = {
      title: (config.options.readOnly ? 'Viewing' : 'Editing') + ' Document: ' + stringDocIDs(req.document._id),
      editorTheme: config.options.editorTheme,
      docLength: toString(req.document).split(/\r\n|\r|\n/).length,
      docString: toString(req.document),
    };
  ...
  exp.deleteDocument = async function (req, res) {
    ...
    await req.collection.deleteOne(req.document).then(() => {
      req.session.success = 'Document deleted! _id: ' + stringDocIDs(req.document._id);
      res.redirect(
