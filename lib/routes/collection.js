...
let routes = function (config) {
  ...
  exp._getProjection = function (req) {
    let projection      = {};
    let jsonProjection  = req.query.projection  || '';
    if (jsonProjection) {
  ...
  exp.viewCollection = function (req, res) {
    ...
    let defaultKey = (config.defaultKeyNames && config.defaultKeyNames[dbName] && config.defaultKeyNames[dbName][collectionName])
      ? config.defaultKeyNames[dbName][collectionName]
      : '_id';
    let edKey = function (doc, defaultKey) {
      let defaultKeyAsArray = defaultKey.split('.');
      let val = doc;
    ...
    let { skip } = query_options;
    let { limit } = query_options;
    let { sort } = query_options;
  ...
  exp.deleteCollection = function (req, res) {
    ...
    let query = exp._buildMongoQuery(req);
    if (query) {
