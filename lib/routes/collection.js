...
const routes = function (config) {
  ...
  exp._getSimpleAggregatePipeline = function (query, queryOptions) {
    const pipeline = Object.keys(query).length > 0
      ? [{ $match: query }]
      : [];
    return exp._getComplexAggregatePipeline(pipeline, queryOptions);
  };

  exp._getComplexAggregatePipeline = function (query, queryOptions) {
    return [
      { $facet: { items: exp._getBaseAggregatePipeline(query, queryOptions) } },
      {
        $project: {
          items: { $slice: ['$items', queryOptions.skip, queryOptions.limit] },
          count: { $size: '$items' },
        },
      },
    ];
  ...
  exp.viewCollection = async function (req, res) {
    try {
      ...
      const queryAggregate = req.query.runAggregate === 'on'
        ? (query.constructor.name === 'Object'
          ? exp._getSimpleAggregatePipeline(query, queryOptions)
          : exp._getComplexAggregatePipeline(query, queryOptions))  // query is an Array
        : exp._getSimpleAggregatePipeline(query, queryOptions);
      const [result] = await req.collection.aggregate(queryAggregate, { allowDiskUse: config.mongodb.allowDiskUse }).toArray();
