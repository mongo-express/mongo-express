...
var routes = function (config) {
  ...
  exp.exportCollection = function (req, res) {
    const filename = `${kebabCase(req.collectionName)}.json`;
    res.setHeader(
      'Content-Disposition',
    ...
    req.collection.find(query, projection, query_options).stream({
      transform: function (item) {
        return EJSON.stringify(item) + os.EOL;
      },
    }).pipe(res);
  };
  ...
  exp.exportColArray = function (req, res) {
    ...
    req.collection.find(query, projection, query_options).toArray(function (err, items) {
      ...
      res.setHeader('Content-Type', 'application/json');
      res.write(EJSON.stringify(items));
      res.end();
    });
  };

  exp.exportCsv = function (req, res) {
    const filename = `${kebabCase(req.collectionName)}.csv`;
    var query;
    var query_options;
    var projection;
  ...
  exp.importCollection = function (req, res) {
    ...
    if (keys.some(key => req.files[key].mimetype !== 'application/json' ||
    ...
    for (const key of keys) {
      ...
      try {
        const arr = EJSON.parse(fileContent);
    ...
    req.collection.insertMany(docs).then((stats) => {
