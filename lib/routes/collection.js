...
import { bytesToSize, buildCollectionURL, roughSizeOfObject } from '../utils.js';
import {
  parseObjectId, toBSON, toJsonString, toSafeBSON, toString,
} from '../bson.js';
...
const routes = function (config) {
  ...
  exp._buildMongoQuery = function (req, res, queryOptions) {
    ...
    const runAggregate = !!req.query.runAggregate;
  ...
  exp._getProjection = function (req) {
    ...
    if (jsonProjection) {
      projection = toSafeBSON(jsonProjection) || {};
    }
    return projection;
  };
  ...
  exp.exportCollection = async function (req, res) {
    ...
    await req.collection.find(query, queryOptions).stream({
      transform(item) {
        return toJsonString(item) + os.EOL;
      },
    }).pipe(res);
  };
