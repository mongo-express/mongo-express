...
const routes = function (config) {
  ...
  exp._getAggregatePipelineCount = function (pipeline) {
    return [
      ...pipeline,
  ...
  exp.viewCollection = async function (req, res) {
    try {
      ...
      if (req.query.runAggregate === 'on' && query.constructor.name === 'Array') {
        ...
        let resCount;
        [items, resCount] = await Promise.all([
          req.collection.aggregate(queryAggregate, { allowDiskUse: config.mongodb.allowDiskUse }).toArray(),
          req.collection.aggregate(exp._getAggregatePipelineCount(query)).toArray(),
        ]);
        count = resCount.at(0)?.count;
      } else {
        queryOptions.allowDiskUse = config.mongodb.allowDiskUse;
