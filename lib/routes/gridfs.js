...
let routes = function () {
  ...
  exp.viewBucket = function (req, res) {
    ...
    let statsAvgChunk  = utils.bytesToSize(files.reduce((prev, curr) => prev + curr.chunkSize, 0) / files.length);
    let statsTotalSize = utils.bytesToSize(files.reduce((prev, curr) => prev + curr.length, 0));
  ...
  exp.addFile = function (req, res) {
    ...
    let newFileID   = new mongo.ObjectId();
    ...
    let gfs         = new GridFSStream(req.db, mongo);
