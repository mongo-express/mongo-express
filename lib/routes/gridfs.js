...
const routes = function () {
  ...
  exp.viewBucket = function (req, res) {
    ...
    const statsAvgChunk  = bytesToSize(files.reduce((prev, curr) => prev + curr.chunkSize, 0) / files.length);
    const statsTotalSize = bytesToSize(files.reduce((prev, curr) => prev + curr.length, 0));
    ...
    for (const f in files) {
      ...
      files[f].length     = bytesToSize(files[f].length);   // Filesizes to something more readable
