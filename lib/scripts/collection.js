    ...
    $.get(`${makeCollectionUrl()}${encodeURIComponent(_id)}/${prop}`, function (input) {
      ...
      if (
        ...
        (
          ...
          input.split(/#|\?/)[0].split('.').pop().trim().toLowerCase() === 'jpeg' ||
          input.split(/#|\?/)[0].split('.').pop().trim().toLowerCase() === 'png'  ||
          input.split(/#|\?/)[0].split('.').pop().trim().toLowerCase() === 'jpg'  ||
          input.split(/#|\?/)[0].split('.').pop().trim().toLowerCase() === 'gif'
        )
      )  {
        input = '<img src="' + entifyGTLTAmp(input) + '" style="max-height:100%; max-width:100%; " alt="' +  entifyGTLTAmp(input) + '/>';
      }
      ...
      if (
        ...
        input = '<audio controls style="width:45px;" src="' + entifyGTLTAmp(input) + '">Your browser does not support the audio element.</audio>';
      }
      ...
      if (
        ...
        input = '<video controls><source type="' + input.substring(input.indexOf(':') + 1, input.indexOf(';')) +
          '" src="' + entifyGTLTAmp(input) + '"/>' +
