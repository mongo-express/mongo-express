...
// credits: https://stackoverflow.com/users/1596799/aliceljm
...
export const colsToGrid = function (cols) {
  ...
  // cols comes in an object of all databases and all their collections return an object of all databases and all potential GridFS x.files & x.chunks
...
// `qs` is used instead of `node:querystring` because it serializes nested objects
// as bracketed keys (`sort[name]=1`), which is what the 'extended' query parser reads back.
export const buildCollectionURL = function (base, dbName, collectionName, queryOptions = {}) {
  const url = buildDatabaseURL(base, dbName) + '/' + encodeURIComponent(collectionName);
...
export const buildDocumentURL = function (base, dbName, collectionName, documentId, queryOptions = {}) {
  const url = buildCollectionURL(base, dbName, collectionName) + '/' + encodeURIComponent(JSON.stringify(documentId));
  const query = qs.stringify(queryOptions);
