...
export const to_display = function (input) {
  ...
  if (
    typeof input === 'string'
    && (
      input.substring(0, 22) === 'data:image/png;base64,'
      || input.substring(0, 22) === 'data:image/gif;base64,'
      || input.substring(0, 22) === 'data:image/jpg;base64,'
      || input.substring(0, 23) === 'data:image/jpeg;base64,'
    )
  )  {
  ...
  if (
    typeof input === 'string'
    && (
      input.substring(0, 22) === 'data:audio/ogg;base64,'
      || input.substring(0, 22) === 'data:audio/mp3;base64,'
    )
  )  {
  ...
  if (
    typeof input === 'string'
    && (
      input.substring(0, 23) === 'data:video/webm;base64,'
      || input.substring(0, 22) === 'data:video/mp4;base64,'
      ||  input.substring(0, 22) === 'data:video/ogv;base64,'
    )
  )  {
...
export const is_URL = function (input) {
  return /^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)[a-z0-9]+([-.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$/.test(input);
};
